...
// 这些字卡不是聊天通用回复，是触发对应功能时联系人才会使用。
  ...
  const DCF_DESC = {
    ...
    care: '【TA的关心（经期）】预测到经期/排卵期前后、或推迟 ≥5 天时，联系人在聊天里发「经期关心」话术（附「经期关心」标签）。\n触发时机：TA 每条文字/表情/图片回复后、打开经期页、保存提醒设置时各判定一次；每个语境每天最多一条；无深夜静默。\n本概率 = 在原有闸门之上统一放行（默认 100%）：在「每天每语境一条」之上，经期中第 1–2 天另有 90%、第 3–4 天 70%、第 5+ 天 55%，经期前提醒/推迟预警 75% 的基数；本项 100% = 保持原节奏，0% = 完全不发关心。\n更彻底：到「经期记录」页把「梦角关心」按钮也关掉（两者都关才真的完全关）。',
