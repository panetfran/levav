// 红底测试：验证 captureScreenshot 是否返回新帧（背景应变红）
...
const chrome = spawn(chromePath, ['--headless=new', '--disable-gpu', '--no-first-run',
  '--user-data-dir=' + join(process.env.TEMP || '/tmp', 'mochi-red-' + Date.now()),
...
const evalJs = async (expr) => { const r = await cdp('Runtime.evaluate', { expression: expr, returnByValue: true }); return r && r.result ? r.result.value : null; };
...
await sleep(600);
await evalJs("(function(){document.querySelectorAll('.page').forEach(function(p){p.hidden=(p.id!=='page-phone');});document.body.style.background='red';return true;})()");
await sleep(500);
const shot = await cdp('Page.captureScreenshot', { format: 'png' });
writeFileSync(join(root, 'tools', 'red-test.png'), Buffer.from(shot.data, 'base64'));
console.log('done');
