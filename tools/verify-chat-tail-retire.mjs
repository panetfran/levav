...
//
// 断言：
//  A 数据级：加几条消息→落盘→重载→再重载，聊天历史条数与内容不增份（日志被回收，不会二次回放）。
//  B 寿命：一次成功权威读库后，chat-tail 里不应再留有「已在历史中」的条目（LS 与 IDB 两副本都空）。
//  C 渲染级：窗口内每个消息节点都带 data-idx（含 msg-poke/msg-rps/msg-center 等非 .msg 类节点），
//    且深翻→脱尾→回底补画后各不重复（本脚本自带最小复现，不依赖 verify-chat-window-sync）。
...
const nAll = await ev('window.getChatMsgs().length');
const dupAll = await ev(`(function(){
  var a=window.getChatMsgs(), seen={}, dup=0;
  a.forEach(function(m){ var k=(m&&m.ts)+'|'+((m&&m.side)||'')+'|'+((m&&m.special)||''); if(seen[k])dup++; else seen[k]=1; });
  return dup;
})()`);
check('A3 整份历史无「同身份(ts|side|special)」重复记录', dupAll === 0, '条数=' + nAll + ' 重复=' + dupAll);
